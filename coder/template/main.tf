...
data "coder_parameter" "home_disk_size" {
  ...
  description  = "Size of the persistent home volume"
  default      = "50"
  type         = "number"
...
SSHKEEPALIVE
    ...
    sudo apt-get install -y -qq --no-install-recommends \
        ...
        libncurses-dev git ca-certificates cpio kmod rsync dwarves \
        >/dev/null 2>&1
    ...
    # Clone kernel source if not already present (persisted on PVC)
    if [ ! -d "$${KERNEL_DIR}/.git" ]; then
        git clone --depth=1 --branch "$${KERNEL_BRANCH}" "$${KERNEL_REPO}" "$${KERNEL_DIR}"
    fi

    # Write build-kernel helper script
    sudo tee /usr/local/bin/build-kernel > /dev/null <<'SCRIPT'
    #!/usr/bin/env bash
    set -eux
    cd $HOME/linux
    rm -rf out
    ...
    echo "=== Workspace ready. Run 'build-kernel' to start kernel compilation ==="
  EOT
